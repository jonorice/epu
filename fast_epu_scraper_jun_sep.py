# -*- coding: utf-8 -*-
"""
Fast Irish EPU Scraper - June to September 2025
For scaling analysis against existing ProQuest data
"""
...
# Date range - June to September 2025
START_DATE = "2025-06-01"
END_DATE = "2025-09-30"

# Output directory (same as before to use existing database)
...
def main() -> None:
    ...
    LOGGER.info("Fast EPU Scraper - June to September 2025")
    ...
    # Show summary
    print("\n" + "=" * 60)
    print("JUNE-SEPTEMBER 2025 RESULTS")
    print("=" * 60)

    cur = conn.execute("""
        SELECT month,
               SUM(CASE WHEN status IN ('ok', 'paywalled_or_empty', 'empty_text') THEN 1 ELSE 0 END) as total,
               SUM(CASE WHEN match = 1 THEN 1 ELSE 0 END) as matched
        FROM articles
        WHERE month >= '2025-06-01' AND month <= '2025-09-01'
    ...
    for row in cur.fetchall():
        share = row[2] / row[1] * 100 if row[1] > 0 else 0
        print(f"{row[0]}: {row[2]} matched / {row[1]} total ({share:.2f}%)")
